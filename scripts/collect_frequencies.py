...
def numeric_suffix(peptide: str):
    """
    Extract the integer after the last underscore in the peptide name.
    If no number is found, return a large sentinel value to push it to the end.
    """
    m = re.search(r"_(\d+)$", peptide)
...
def main():
    ...
    # Sort by peptide identifier
    result = result.sort_values(by="Peptide", key=lambda col: col.map(numeric_suffix), ascending=True)
